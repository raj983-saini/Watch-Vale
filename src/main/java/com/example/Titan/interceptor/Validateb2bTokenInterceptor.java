...
@Slf4j
@Component
public class Validateb2bTokenInterceptor implements HandlerInterceptor {

    private final UserTokenService userTokenService;
    private final UserService userService;
    private final RoleService roleService;

    public Validateb2bTokenInterceptor(UserTokenService userTokenService, UserService userService, RoleService roleService) {
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response,
        ...
        log.info("Pre handle Interceptor of Validateb2bTokenInterceptor for token {}", request.getHeader("token"));
            ...
            if (ObjectUtils.isEmpty(token)) {
                ...
                try {
                    ...
                    if(role.get().isAllowOtpLogin()){
                        ...
                        log.info("b2b login not allowed in {} error for {}", role, users);
                        sendFailureResponse(response, UNAUTHORIZED);
                        return false;
                    }
                } catch (Exception ue) {
